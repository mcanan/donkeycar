...
def my_model(input_shape):
    ...
    model.compile(optimizer=optimizer,
                  ...
                  loss_weights={'angle_out': 0.95, 'throttle_out': .05})
...
def nvidia_categorical(input_shape):
    img_in = Input(shape=input_shape, name='img_in')
...
def donkey_categorical(input_shape):
    img_in = Input(shape=input_shape, name='img_in')
    x = Convolution2D(24, (5, 5), strides=(2, 2), activation='relu')(img_in)
    ...
    model.compile(optimizer='adam',
                  loss={'angle_out': 'categorical_crossentropy',
                        'throttle_out': 'mean_absolute_error'},
                  loss_weights={'angle_out': 0.9, 'throttle_out': .05})
